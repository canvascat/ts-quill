...
export default class Clipboard extends Module {
  ...
  dangerouslyPasteHTML(index, html, source = Sources.API) {
    if (typeof index === 'string') {
      const delta = this.convert({ html: index, text: '' })
      this.quill.setContents(delta, html)
      this.quill.setSelection(0, Sources.SILENT)
    } else {
      const paste = this.convert({ html, text: '' })
      this.quill.updateContents(new Delta().retain(index).concat(paste), source)
      this.quill.setSelection(index + paste.length(), Sources.SILENT)
    }
  }
  ...
  onPaste(e, range) {
    ...
    this.quill.setSelection(delta.length() - range.length, Sources.SILENT)
...
export function traverse(scroll, node, elementMatchers, textMatchers, nodeMatches) {
  // Post-order
...
export function matchAttributor(node, delta, scroll) {
...
export function matchBlot(node, delta, scroll) {
...
function matchIndent(node, delta, scroll) {
  ...
  return delta.compose(new Delta().retain(delta.length() - 1).retain(1, { indent }))
}
