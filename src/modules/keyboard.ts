  ...
  bindings: {
    ...
    indent: {
      ...
      handler(range, context) {
        ...
        this.quill.format('indent', '+1', Sources.USER)
        return false
      }
    },
    outdent: {
    ...
    'outdent backspace': {
      ...
      handler(range, context) {
        if (context.format.indent != null) {
          ...
          this.quill.format('list', false, Sources.USER)
        }
      }
    },
    ...
    'remove tab': {
      ...
      prefix: /\t$/,
      handler(range) {
        this.quill.deleteText(range.index - 1, 1, Sources.USER)
      }
    },
    tab: {
      key: 'Tab',
      handler(range, context) {
        ...
        this.quill.history.cutoff()
        this.quill.setSelection(range.index + 1, Sources.SILENT)
        return false
      }
    },
    'blockquote empty enter': {
      ...
      empty: true,
      handler() {
        this.quill.format('blockquote', false, Sources.USER)
      }
    },
    'list empty enter': {
      key: 'Enter',
      ...
      handler(range, context) {
        ...
        if (context.format.indent) {
          this.quill.format('indent', false, Sources.USER)
        }
      }
    },
    'checklist enter': {
      ...
      handler(range) {
        ...
        if (module) {
          ...
          if (shift < 0) {
            ...
            this.quill.updateContents(delta, Sources.USER)
            this.quill.setSelection(range.index + 1, range.length, Sources.SILENT)
          } else if (shift > 0) {
            index += table.length()
            const delta = new Delta().retain(index).insert('\n')
            this.quill.updateContents(delta, Sources.USER)
            this.quill.setSelection(index, Sources.USER)
          }
        }
      }
    },
    'list autofill': {
      ...
      handler(range, context) {
        ...
        this.quill.insertText(range.index, ' ', Sources.USER)
        this.quill.history.cutoff()
        const delta = new Delta()
        ...
        this.quill.updateContents(delta, Sources.USER)
        this.quill.history.cutoff()
        this.quill.setSelection(range.index - length, Sources.SILENT)
        return false
      }
    },
    'code exit': {
...
function handleBackspace(range, context) {
  ...
  if (context.offset === 0) {
    ...
    if (prev != null) {
      if (prev.statics.blotName === 'table') {
        this.quill.setSelection(range.index - 1, Sources.USER)
        return
  ...
  this.quill.deleteText(range.index - length, length, Sources.USER)
  if (Object.keys(formats).length > 0) {
    this.quill.formatLine(range.index - length, length, formats, Sources.USER)
  }
  this.quill.focus()
}
...
function handleDelete(range, context) {
  ...
  this.quill.deleteText(range.index, length, Sources.USER)
  if (Object.keys(formats).length > 0) {
    this.quill.formatLine(range.index + nextLength - 1, length, formats, Sources.USER)
  }
}
...
function handleDeleteRange(range) {
  ...
  if (Object.keys(formats).length > 0) {
    this.quill.formatLine(range.index, 1, formats, Sources.USER)
  }
...
function handleEnter(range, context) {
  ...
  Object.keys(context.format).forEach(name => {
    ...
    if (name === 'link') return
    this.quill.format(name, context.format[name], Sources.USER)
  })
}
...
function makeCodeBlockHandler(indent) {
  return {
    ...
    handler(range) {
      ...
      this.quill.setSelection(index, length, Sources.SILENT)
    }
  }
}
...
function makeEmbedArrowHandler(key, shiftKey) {
  ...
  return {
    ...
    handler(range) {
      ...
      if (key === 'ArrowLeft') {
        if (shiftKey) {
          this.quill.setSelection(range.index - 1, range.length + 1, Sources.USER)
        } else {
          this.quill.setSelection(range.index - 1, Sources.USER)
        }
      } else if (shiftKey) {
        this.quill.setSelection(range.index, range.length + 1, Sources.USER)
      } else {
        this.quill.setSelection(range.index + range.length + 1, Sources.USER)
      }
      return false
...
function makeFormatHandler(format) {
  return {
    ...
    shortKey: true,
    handler(range, context) {
      this.quill.format(format, !context.format[format], Sources.USER)
    }
  }
}
    ...
    handler(range, context) {
      ...
      if (targetRow != null) {
        if (targetRow.statics.blotName === 'table-row') {
          ...
          this.quill.setSelection(index, 0, Sources.USER)
        }
      } else {
        ...
        if (targetLine != null) {
          if (up) {
            ...
            this.quill.setSelection(targetLine.offset(this.quill.scroll), 0, Sources.USER)
          }
        }
      }
      return false
